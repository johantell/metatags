defmodule MetatagsTest do
  ...
  test "prints a metatag as a safe string" do
    result =
      %{metadata: %{"description" => "cool stuff"}}
      |> Metatags.print_tags()
      |> to_binary
  ...
  test "inherits og:url from canonical when og:url is nil" do
    result =
      %{metadata: %{"canonical" => "/path", og: %{"url" => nil}}}
      |> Metatags.print_tags()
      |> to_binary

    assert result ==
             ~s(<meta content="/path" name="canonical"><meta content="/path" name="og:url">)
  end
end
